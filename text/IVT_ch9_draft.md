...
在第六章讨论的九个理论框架——Rochallyi 向量诗歌[1]、Gärdenfors 双向量模型[13]、Jacobs NCPM[15]、Stockwell 认知诗学[17]、层论表征[30]、洛特曼符号圈[5]、Text World Theory[20]、SentiArt[16]、POET[29] ——等工具性框架中，意象向量论是**少数同时使用向量/几何语言、讨论维度锁定、讨论读者责任转移、并且可作为分析工具的框架**。这一独特定位说明意象向量论填补了一个整合性的学术位置。附录 B（蟛蜞创作精选分析）和附录 C（中外名篇精选分析）中 ivt-poem-analyzer 的分析结果，是这一解释力的初步验证。但它真正的学术价值，最终将由更广泛的独立研究在更多诗例上的应用来检验。
