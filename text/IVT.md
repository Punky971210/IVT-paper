...
- **2.3 意象向量的三维属性**：
  - **方向**：由文化编码预先染色，如雄鹰指向力量或昂扬、残阳指向衰落或终结
  - **模长**：由意象的感知强度决定，例如"枯藤"模长大于"藤"，因为"枯"强化了方向
...
- **2.6 向的两种来源：文化编码与结构赋值的同标异向**
  - 传统意象的"向"依赖**文化编码**，"枯"的衰败方向在语言共同体中预先染好，作者只需调用既有的文化储备即可，对于训练有素的诗读者来说也很容易迅速唤起。
  - 蟛蜞式意象的"向"依赖**结构赋值**，"耳机隔绝世界"、"人来人往的世界很大/很挤"，标的"向"不是在文化中预先包装好的，而是在文本内部由语言结构的排列产生的
  - **同标异向**：同一个标的被赋两个相反的向。"人来人往的世界很大/挤得下我的卑微身影，人来人往的世界很挤/容不下我的竭力呼喊"，"世界"这个标的在四句中被赋了两个方向：一方容纳卑微身影，另一方拒绝竭力呼喊。同一标在不同位置被赋不同向，读者的运算不是在"枯/残"的单一方向上做加法，而是在两个方向之间做切换
  - **理论意义**：前者是"调用文化储备"，作者从文化储备中提取已染色的意象；后者是"构造语言结构"，作者通过语言排列本身产生方向性，不需要调用任何传统意象储备。IVT的价值在于两种来源均可被该框架分析，且分析后者时拓展了传统诗学"意象"概念的分析边界
...
- **5.4 验证条件**：删去所有情绪词后，物象是否仍能独立支撑完整情感场
  - 案例检验：蟛蜞《调试日志》删去情绪用词后，情感场完整
...
> **与第七章**：本标准模型（第2至6章）→ 嵌套扩展（第7章），后者打破"我=隐含参考系原点"的假设，将IVT从意象层面扩展到主体层面。
...
#### 7.9.7 与现有IVS结构的关系
嵌套空间论回答"是否存在嵌套"，切换分析回答"构建状态如何动态变化"，两者互补。分析流程：检测嵌套激活——激活时执行嵌套分析与切换分析，未激活时检查视角切换后回归标准IVT分析。

#### 7.9.8 对读者情感劳动的影响
- 新增第4层运算——构建状态切换这一模式切换运算
...
- **9.3 学术空白定位**：现有学术文献中，尚缺乏将诗学、认知科学、计算机科学、数学四个分散传统整合为"标→向→移动→维度锁定→认知渲染→读者推测"的完整因果链。而创作实践作为理论源头，进一步确保该框架以真实的创作直觉为翻译基础，形成可操作模型而非纯理论的推演
## 9.4 未来方向
...
分析工具的成型与验证，使IVT框架完成了从理论假设到可重复分析操作的跨越。当前的核心方向已从分析层面的验证转向**生成算法层面的研究**，将IVT的核心主张从分析框架转化为生成算法的约束条件。
...
第五章提出的负优化概念，有意识地不提供足量向量信息以增加读者端的运算量这一写作方式在生成算法中具有独特的应用价值。核心问题在于：如何量化一首诗的"留白程度"？如何在生成过程中精确控制"向"信息的提供量，使读者端的运算量处于最优区间？可能的探索路径包括：通过向量密度估算 $A_{\text{候选}}$、通过异频夹角总和估算方向不确定性、以及通过动词-动量的方向一致性量化轨迹的收敛程度。
...
> **注**：以上五个方向属于探索性拟定，后续版本中需根据生成算法实验的初步结果进行收敛与调整。当前IVT分析工具的批量化验证使算法研究有了可靠的评估基准，生成结果是否成功，可通过既有的IVT分析工具做逆向检验：生成文本的标/向分离是否合理、向量方向是否一致、读者责任转移是否处于最优区间。
